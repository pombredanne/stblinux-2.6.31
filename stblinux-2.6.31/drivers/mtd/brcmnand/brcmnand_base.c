...
// Only True when CONFIG_MTD_BRCMNAND_GLOBAL_LOCK
static int gGlobalLock = 0;
 ...
 */
	...
	{	/* 7b new generation, same ID, different decode-ID table */
		.chipId = HYNIX_HY27UF082G2B,
		.chipId345 = {0x10, 0x95, 0x44}, 
		.mafId = FLASHTYPE_HYNIX,
...
/* New Chip ID scheme in place and working, but as of 2631-2.5 these do not work yet, for some unknown reason */
	...
	{	/* 39 */
		.chipId = HYNIX_HY27U518S2C,
		.mafId = FLASHTYPE_HYNIX,
		.chipIdStr = "HYNIX HY27U518S2C",
		.options = NAND_USE_FLASH_BBT,
		.idOptions = 0,
		.timing1 = 0, .timing2 = 0,
		.ctrlVersion = 0,
	...
	{	/* 40 */
		.chipId = MACRONIX_MX30F1G08,
		.mafId = FLASHTYPE_MACRONIX,
		.chipIdStr = "MACRONIX MX30F1G08",
		.options = NAND_USE_FLASH_BBT, 		/* Use BBT on flash */
		.idOptions = BRCMNAND_ID_EXT_BYTES,
		.timing1 = 0, 
		.timing2 = 0,
...
static const unsigned char ffchars[] = {
	...
	0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,	/* 240 */
	0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,	
	0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,	/* 512 */
};
...
static int brcmnand_wait(struct mtd_info *mtd, int state, uint32_t* pStatus)
{
	...
	while (time_before(jiffies, timeout)) {
		if (!gGlobalLock && state != FL_READING && (!wr_preempt_en) && !in_interrupt())
			cond_resched();
		else
			udelay(10);
...
printk("-->%s, raw=%d\n", __FUNCTION__, raw);}
	...
	/* The 1 msec is enough */
	timeout = jiffies + msecs_to_jiffies(1);  // 1 msec timeout for now
	PLATFORM_IOFLUSH_WAR();
	ready = chip->ctrl_read(BCHP_NAND_INTFC_STATUS);
 ...
 * BRCMEDU_MEM_BUS_ERROR				(-5)
 */
static int brcmnand_NEDU_cache_is_valid(struct mtd_info* mtd,  int state, loff_t offset, uint32_t intr_status) 
{
	//struct brcmnand_chip * chip = mtd->priv;
	int error = 0;
	//unsigned long flags;
...
printk("%s: intr_status=0 TIMEOUT\n", __FUNCTION__);
	...
	else if (intr_status & BCHP_HIF_INTR2_CPU_STATUS_NAND_UNC_INTR_MASK)
	{
		error = BRCMNAND_UNCORRECTABLE_ECC_ERROR;
	}
	else if (intr_status & BCHP_HIF_INTR2_CPU_STATUS_NAND_CORR_INTR_MASK)
	{
		error = BRCMNAND_CORRECTABLE_ECC_ERROR;
	}
...
//out:
 	...
 	// Clear error status
	BDEV_WR(BCHP_HIF_INTR2_CPU_CLEAR, HIF_INTR2_EDU_CLEAR_MASK);    
...
static int (*brcmnand_posted_read_cache)(struct mtd_info*, 
  ...
  #else /* EDU and no EDU Queue Mode */
static int (*brcmnand_posted_read_cache)(struct mtd_info*, 
		void*, u_char*, loff_t) = brcmnand_ctrl_posted_read_cache;
  #endif

/* End #ifdef CONFIG_MTD_BRCMNAND_EDU */
...
/*
 * Read completion after ISR_READ is called.
 * In ISR mode, this routine is run in interrupt context
 */
int
        ...
        void* buffer, u_char* oobarea, loff_t offset, uint32_t intr_status)
{
	...
	if (intr_status & HIF_INTR2_EDU_ERR) {
		printk("%s: Should not call me with ERR\n", __FUNCTION__);
		BUG();
	}
	intfc_status = chip->ctrl_read(BCHP_NAND_INTFC_STATUS);
	while (!(intfc_status & BCHP_NAND_INTFC_STATUS_CTLR_READY_MASK) && retries > 0) {
...
/*
 * No DMA, but interrupt driven flash interface
 * Just clear the error and return.
 */
...
// Also for ISR no-EDU case
static int (*brcmnand_posted_write_cache)(struct mtd_info*, 
		const void*, const u_char*, loff_t) = brcmnand_ctrl_posted_write_cache; 

  

#elif defined  (CONFIG_MTD_BRCMNAND_EDU) /* EDU but no ISR Queue */

/*
 * Write completion after EDU_Write is called.
 * Non-Queue mode
 */
static int
...
#else /* No ISR Queue (EDU or ISR-noEDU) */
static int (*brcmnand_posted_write_cache)(struct mtd_info*, 
		const void*, const u_char*, loff_t) = brcmnand_ctrl_posted_write_cache;

#endif  //  ISR Queue used 
 ...
 *
 * Get the device and lock it for exclusive access.  Interrupts are also disabled
 * Cannot be run in EDU/ISR mode.
 */
...
static int brcmnand_get_device(struct mtd_info *mtd, int new_state)
{
	...
	if (chip) {
		 ...
		 */
		while (1) {
			spin_lock_irqsave(&chip->ctrl->chip_lock, chip->ctrl->lockflags);
			...
			if (new_state == FL_PM_SUSPENDED) {
				spin_unlock_irqrestore(&chip->ctrl->chip_lock, chip->ctrl->lockflags);
...
static int brcmnand_release_device(struct mtd_info *mtd)
{
	struct brcmnand_chip * chip = mtd->priv;
...
#else
	/*  
	 * Non EDU ISR procedures
	 */


NEDU_issue_command(struct brcmnand_chip* chip, int command) 
{
	// Enable all L2 interrupts
	HIF_ENABLE_IRQ(NAND_CTLRDY);
	HIF_ENABLE_IRQ(NAND_UNC);
	HIF_ENABLE_IRQ(NAND_CORR);
	HIF_ENABLE_IRQ(EBI_TIMEOUT);

	if (NEDU_READ == command) {
		chip->ctrl_write(BCHP_NAND_CMD_START, OP_PAGE_READ);
	}
	else if (NEDU_WRITE == command) {
		chip->ctrl_write(BCHP_NAND_CMD_START, OP_PROGRAM_PAGE);
	}
}
 ...
 */
static void 
NEDU_submit_read(eduIsrNode_t* req)
{
	struct brcmnand_chip* chip = (struct brcmnand_chip*) req->mtd->priv;
	uint32_t hif2_status;
	uint32_t* p32;
	loff_t sliceOffset = req->offset & (~ (ECCSIZE(req->mtd) - 1));
	int i, ret = 0;
	int retries = 5, done=0;
	int valid = 0;

PRINTK("-->%s offset=%llx\n", __FUNCTION__, req->offset);
...
if (gdebug > 3 ) 
{printk("%s: offset=%0llx, oobarea=%p\n", __FUNCTION__, req->offset, req->oobarea);}


	if (unlikely(req->offset - sliceOffset)) {
		printk(KERN_ERR "%s: offset %08x is not cache aligned, sliceOffset=%08lx, CacheSize=%d\n", 
			__FUNCTION__, (unsigned int) req->offset, (unsigned long) sliceOffset, ECCSIZE(req->mtd));
		return -EINVAL;
	}
	...
	NEDU_issue_command(chip, NEDU_READ);
...
int NEDU_submit_write(eduIsrNode_t* req)
{
...
PRINTK("-->%s offset=%llx\n", __FUNCTION__, req->offset);
	...
	if (unlikely(sliceOffset - req->offset)) {
		printk(KERN_ERR "%s: offset %0llx is not cache aligned\n", 
			__FUNCTION__, req->offset);
	...
	if (req->buffer) {
if (gdebug > 3 ) {print_databuf(req->buffer, 32);}
		brcmnand_to_flash_memcpy32(chip, req->offset, req->buffer, ECCSIZE(req->mtd));
	}
#if 0 
	/* Must write data when NAND_COMPLEX_OOB_WRITE */
	else if (chip->options & NAND_COMPLEX_OOB_WRITE) {
...
//printk("50\n");
	...
	NEDU_issue_command(chip, NEDU_WRITE);
...
PRINTK("%s: Submit job offset=%0llx, for %s\n", __FUNCTION__, req->offset, 
gJobQ.cmd == EDU_READ? "READ" : "WRITE");
			   ...
			   const uint8_t* inp_oob, uint64_t page)
{
	...
	/*
	 * SWLINUX-1777: WHen partial-page-en=0, must write with data
	 * Relies on PARTIAL_PAGE_EN bits being at the same position between CS0 & CSn
	 */
	uint32_t acc = chip->ctrl_read(bchp_nand_acc_control(chip->ctrl->CS[chip->csi]));
	int mustWriteWithData = 0;

	if (0 == (acc & BCHP_NAND_ACC_CONTROL_PARTIAL_PAGE_EN_MASK))
	{
		mustWriteWithData = 1;
	}
	...
	for (eccstep = 0; eccstep < chip->eccsteps && ret == 0; eccstep++) {
		if (!mustWriteWithData) {
			ret = brcmnand_posted_write_oob(mtd,  &inp_oob[oobWritten] , 
					offset);
		}
		else {
			ret = brcmnand_ctrl_posted_write_cache(mtd,  &ffchars[0], &inp_oob[oobWritten] , 
					offset);
		}
//gdebug=0;		
		if (ret < 0) {
 ...
 * for Small Page NAND, just return hard-coded values.
 */
uint32_t
...
PRINTK("-->%s, chip->chipSize=%llx\n", __FUNCTION__, chip->chipSize);
	colAdrBytes = 2; // Not used with small page devices
	...
	if (chip->pageSize == 512) {
		// blkAdrBytes = 3; /* 64MB and 128MB */
		fulAdrBytes = blkAdrBytes + 1; /* Most mfg only use 5-bits (A0-A4) for col addr. */
	}
...
/*
 * Set NAND_ACC_CONTROL register
 * For flash identified by Read-ID method, nbrBitsPerCell=0 in order to skip recreaiting cellinfo.
 */
...
printk("eccLevel=%d, 1Ksector=%d, oob=%d\n", eccLevel, b1Ksector, oobPerSector);
	...
	/* Fabricate cellinfo, only if not using Samsung ID table (byte 3) */
	if (nbrBitsPerCell != 0) {
		/*
...
PRINTK("%s: gAccControl[CS=%d]=%08x, ACC=%08lx\n", 
	__FUNCTION__, cs, gAccControl[chip->csi], nand_acc_b4);
	 ...
	 /* Update ACC-CONTROL when not on CS0 */
	 	...
	 	int nbrBitsPerCells = 0; // Cellinfo already parsedusing ID table, no need to redo in set_acccontrol()
	 	int oobSizePerPage = chip->eccOobSize*(chip->pageSize/512);

		brcmnand_set_acccontrol(chip , cs, 
			chip->pageSize, oobSizePerPage, chip->reqEccLevel, chip->eccSectorSize,  nbrBitsPerCells);
	 }
...
/*---------------- 5th ID byte ------------------------- */
	...
	PRINTK("nbrPlanes = %d\n", nbrPlanes);
		...
		switch(brcmnand_maf_id) {
		case FLASHTYPE_SAMSUNG:
		//case FLASHTYPE_MACRONIX:  // Does not have 5th ID byte
		case FLASHTYPE_MICRON:
...
decode_ID_type2(struct brcmnand_chip * chip, 
	...
	uint32_t* pEccLevel, int* pSectorSize)
{
	uint32_t nand_config = chip->ctrl_read(bchp_nand_config(chip->ctrl->CS[chip->csi])); // returned value
	unsigned char devId4thdByte =  (chip->device_id  & 0xff);
	unsigned int pageSize = 0, pageSizeBits = 0;
...
PRINTK("Required ECC level = %d, devIdExt=%08x, eccShift=%02x, sector Size=%d\n", 
...
static int brcmnand_monitor_intfc(struct brcmnand_chip * chip, brcmnand_ctrl_state_t waitfor, uint32_t* pStatus)
{
	unsigned long timeout;
	uint32_t ready;
...
//gdebug = 0;
	...
	timeout = jiffies + msecs_to_jiffies(1000); // THT: 1000 msec, for now
	while (time_before(jiffies, timeout) ) {
		switch (state) {
...
brcmnand_chip_reset(struct brcmnand_chip* chip, int chipSelect, int* pStatus)
{
	...
	ret = brcmnand_monitor_intfc(chip, BRCMNAND_CTRL_READY, pStatus);
	return ret;
 ...
 *
  * if chipSelect != 0, we need to read the Primary and Extended pages in order to find out the required ECC level
  * Otherwise, when CS=0, the bootloader/HW have already configured the NAND_CONFIG and NAND_ACC_CONTROL
  *	registers, so we just detect ONFI signature and return success.
 */
static int
...
PRINTK("B4 PARAM READ, nand_config0=%08x, nand_config1=%08x, ret=%d\n", nand_config0, nand_config, ret);	
		...
		ret = brcmnand_monitor_intfc(chip, BRCMNAND_CACHE_VALID, &status);
...
PRINTK("EccLevel = [%08x], %02x, pageSize=%d, oobSize=%d\n", u32, eccLevel, *outp_pageSize, *outp_oobSize);
	...
	if (chipSelect == 0) {
		/* On CS0, nothing done, controller already populate nand_config and nand_acc */
...
static int brcmnand_probe(struct mtd_info *mtd, unsigned int chipSelect)
{
	...
	/* else we don't overwrite NAND_CONFIG when on CS0 */
	
#endif

	/* 
	 * Starts with Read-ID, then fall back to ONFI to avoid timeout due to ONFI discovery
	 */
...

	{
		...
		ret = brcmnand_chip_reset(chip, chipSelect, &status);
		...
		chip->device_id = chip->ctrl_read(BCHP_NAND_FLASH_DEVICE_ID);
	}
#else
	...
	if (!foundInIdTable || i >= BRCMNAND_MAX_CHIPS) {
...
#else
		printk(KERN_WARNING "DevId %08x not in ID table.  Will try ONFI\n", (unsigned int) chip->device_id);	
		...
		if (isONFI) { /* ONFI capable */
			...
			else { // Make sure that ACC is correct
				nand_config = chip->ctrl_read(bchp_nand_config(0));
				printk("NAND_CONFIG=%08x, NAND_ACC=%08x\n", 
					nand_config,
					chip->ctrl_read(bchp_nand_acc_control(0)));
			}
  ...
  #endif // if ONFI capable
  		{
			printk(KERN_WARNING "DevId %08x not in ID table.  Will use value set in NAND_CONFIG\n", (unsigned int) chip->device_id);		
  		}
#endif // NAND version > 0.0


	}

	if (isONFI || foundInIdTable) {
		// Record NOP if known
...
PRINTK("chip->cellinfo=%02x\n", chip->cellinfo);
	...
	printk("%s: Ecc level set to %d, sectorSize=%d from ID table\n", __FUNCTION__, chip->reqEccLevel, chip->eccSectorSize);
		...
		if (!skipIdLookup && !foundInIdTable) {
			uint32_t acc;
...
printk("ECC layout=brcmnand_oob_16\n");
			}
			else if (chip->ecclevel == BRCMNAND_ECC_BCH_4) {
