...
#ifdef CONFIG_SMP
static void brcm_intc_set_affinity(unsigned int irq, const struct cpumask *dest)
{
	unsigned int shift;
	unsigned long flags;
...
static struct irq_chip brcm_upg_type = {
	name: "BRCM UPG L2",
	enable: brcm_upg_enable,
	disable: brcm_upg_disable,
	NULL
};
...
void __init arch_init_irq(void)
{
	...
	for (irq = BRCM_UPG_L2_BASE; irq <= BRCM_UPG_L2_LAST; irq++)
		set_irq_chip(irq, &brcm_upg_type);
