		...
		void* buffer, u_char* oobarea, loff_t offset)
{
	...
	u_char* __attribute__((unused)) p8 = (u_char*) p32;
	//unsigned long irqflags;	
...
// printk("EDU10\n");
	...
	if (unlikely(!EDU_buffer_OK((volatile void *)buffer))) {
...
// #else re-use for EDU
	...
	if (unlikely(gdebug > 0)) {
		printk("DEBUG -> erasing %0llx, %lx %d\n",
			(unsigned long long)instr->addr,
			(unsigned long)instr->len, this->state);
	}
			...
			case BRCMNAND_ECC_BCH_8:
				...
				if (chip->eccOobSize < eccOobSize) {
					printk("Flash says it has %d OOB bytes, but ECC level %d need %d bytes\n",
						chip->eccOobSize, (int)eccLevel, eccOobSize);
